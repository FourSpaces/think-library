...
<body>
  ...
  <ul class="breadcrumb">
    ...
    <li class="active"><?php if(($titleTab) == "1"): ?>添加<?php else: ?>编辑<?php endif; ?>文章</li>
  </ul>
...
<div class="container-fluid">
  ...
  <div class="well">
     <form class="demoform" action="<?php echo U('');?>">
      <input name='id' type="hidden" value="<?php echo ((isset($info["id"]) && ($info["id"] !== ""))?($info["id"]):''); ?>">
      <ul style="list-style-type: none;">
        <li>
          <label>文章标题</label>
          <input  name="title" type="text" value="<?php echo ((isset($info["title"]) && ($info["title"] !== ""))?($info["title"]):''); ?>" class="input-xlarge" datatype="*2-40" errormsg="文章标题 2~40个字符！" sucmsg="&nbsp;">
          <span class="Validform_checktip"></span>
        </li>
        <li>
          <label>文章简介</label>
          <textarea name="summary" class="input-xlarge" datatype="*" errormsg="文章简介不能为空！" sucmsg="&nbsp;"><?php echo ((isset($info["summary"]) && ($info["summary"] !== ""))?($info["summary"]):''); ?></textarea>
          <span class="Validform_checktip"></span>
        </li>
        <li>
          <label>文章分类</label>
          <select name="type" class="input-xlarge" datatype="*" errormsg="请选择文章分类" sucmsg="&nbsp;">
            ...
            <?php if(empty($info["type"])): if(is_array($sort)): $i = 0; $__LIST__ = $sort;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["id"]); ?>'><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php else: ?>
              <?php if(is_array($sort)): $i = 0; $__LIST__ = $sort;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["id"]); ?>' <?php if(($vo["id"]) == $info["type"]): ?>selected="selected"<?php endif; ?> ><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
...
<script type="text/javascript">
  ...
  <!--
    <script type="text/javascript" src="/think-library/Public/js/jquery-2.0.2.js"></script>
  -->
    <script type="text/javascript" src="/think-library/Public/ueditor/ueditor.config.js"></script>    
    <script type="text/javascript" src="/think-library/Public/ueditor/ueditor.all.min.js"></script>
  <script>
  $(function() {
