...
<div class="container-fluid">
  <div class="row-fluid">            
    ...
    <div class="btn-toolbar">
        <a href="<?php echo U('Admin/Bookmanage/addstyle');?>" class="btn btn-primary"><i class="icon-plus"></i>添加类型</a>
        <!--
        ...
        <table class="table">
          ...
          <tbody>
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
                <td><?php echo ($i); ?></td>
                <td><?php echo ($order["typename"]); ?></td>
                <td><?php echo ($order["sort"]); ?></td>            
                <td><?php echo (date('Y-m-d H:i',$order["updatetime"])); ?></td>
                <td>
                  <a href="<?php echo U('Admin/Bookmanage/editstyle',array('id'=>$order['id']));?>" title="编辑"><i class="icon-pencil"></i></a>&nbsp;&nbsp;&nbsp;
                  <!-- href="#myModal"  -->
  ...
  <script>
    function deletestyle(value){
          ...
          $("#modal_delete").on("click",function(){
            //在点击按钮后延迟动作
            setTimeout(function(){
              $.post("<?php echo U('Admin/Bookmanage/deletestyle');?>",{ id:value,},
