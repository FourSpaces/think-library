...
<body>
  ...
  <ul class="breadcrumb">
    ...
    <li class="active"><?php if(($titleTab) == "1"): ?>添加<?php else: ?>编辑<?php endif; ?>出版社</li>
  </ul>
...
<div class="container-fluid">
  ...
  <div class="well">
     <form class="demoform" action="<?php echo U('');?>">
      <input name='typeid' type="hidden" value="<?php echo ((isset($info["id"]) && ($info["id"] !== ""))?($info["id"]):''); ?>">
      <ul style="list-style-type: none;">
        <li>
          <label>出版社名称：</label>
          <input id="input_pressname" name="pressname" type="text" value="<?php echo ((isset($info["pressname"]) && ($info["pressname"] !== ""))?($info["pressname"]):''); ?>" class="input-xlarge" datatype="s2-16" errormsg="分类名称至少2个字符,最多16个字符！" sucmsg="&nbsp;">
          <span class="Validform_checktip"></span>
        </li>
        <li>
          <label>出版社排序：</label>
          <input id="input_sort" name="sort" type="number" value="<?php echo ((isset($info["sort"]) && ($info["sort"] !== ""))?($info["sort"]):'0'); ?>" class="input-xlarge" datatype="*" errormsg="排序值不能为空" sucmsg="&nbsp;">
          <span class="Validform_checktip"></span>
        </li>
        <li>
